...
def collectResults(recreate_csv = False, key = 'df', files = []):
    '''Collect the results and put them in a dataframe.  Then can look at results based on performance of collections of parameters.'''
...
def main(args):
    parser = argparse.ArgumentParser('Plot the training results and get some stats about the cross validation.')
    parser.add_argument('--filter', default = 'jz5_bkg_v1', dest='filter_id', help = 'Filter and remove file names on this string. Default jz5_bkg_v1.')
    parser.add_argument('--key', default = 'mc15_v2.2_bkg_v1', dest='key', help = 'Filter and select file names on this string. Default mc15_v2.2_bkg_v1.')
    parser.add_argument('--recreate-csv', dest = 'recreate_csv', action = 'store_true', help = 'If the cross validation stats pandas dataframe should be recreated, or if it can just be read in from a previous csv file.')
    parser.set_defaults(recreate_csv = False)
    args = parser.parse_args()

    #full_dataset = 'persist/data_features_nc_2_10_v2_100.pkl'
    files = [f for f in os.listdir('evaluationObjects/') if f.find(args.key)!=-1 and f.endswith('.pickle') and f.find(args.filter_id) == -1]

    collectResults(args.recreate_csv, args.key, files)
