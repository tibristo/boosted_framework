...
for x in `ls *mc15_nonTrk_v3_bkg_v1*.yaml` ; do l=`grep cv $x | head -1`; cv=${l:(-6):1}; sed -i '/test_cv.*root/c\  file: /Disk/ds-sopa-group/PPE/atlas/users/tibristo/boosted_framework/WTaggingNN/folds/AntiKt10LCTopoTrimmedPtFrac5SmallR20_13tev_mc15_nonTrk_notcleaned_v3_400_1200_mw_merged_full_scaled_cv_00'${cv}'full.root' $x ; done
...
for x in `ls *mc15_nonTrk_v3_bkg_v1*.yaml` ; do sed -i '/sigeff/c\sigeff: 0.6907' $x ; done
...
for x in `ls *mc15_nonTrk_v3_bkg_v1*.yaml` ; do sed -i '/bkgeff/c\bkgeff: 0.0936' $x ; done
