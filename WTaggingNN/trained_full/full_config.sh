...
for x in `ls *jz5_bkg_v2*.yaml` ; do l=`grep cv $x | head -1`; cv=${l:(-6):1}; sed -i '/test_cv.*root/c\  file: /Disk/ecdf-nfs-ppe/atlas/users/tibristo/boosted_framework/WTaggingNN/folds/AntiKt10LCTopoTrimmedPtFrac5SmallR20_13tev_mc15_jz5_notcleaned_v1_800_1200_mw_merged_full_scaled_cv_00'${cv}'full.root' $x ; done

#for x in `ls *bdt*.yaml` ; do sed -i 's/notcleaned/loose/g' $x ; done
# change the signal eff
for x in `ls *jz5_bkg_v2*.yaml` ; do sed -i '/sigeff/c\sigeff: 0.6978' $x ; done
...
for x in `ls *jz5_bkg_v2*.yaml` ; do sed -i '/bkgeff/c\bkgeff: 0.1196' $x ; done
